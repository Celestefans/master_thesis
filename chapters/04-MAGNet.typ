...
为了获得更加紧凑且适用于 GISR 任务的提示表示，本文不再直接使用混合特征，而是将其作为一个查询信号，去检索一组可学习的语义锚点。本文预定义了一个任务无关的共享提示字典 $cal(D) = {bold(d)_1, bold(d)_2, dots, bold(d)_K}$，其中 $bold(d)_k in RR^D$ 为第 $k$ 个潜在的语义原子（Semantic Atom），$K$ 为原子总数。
本文将 $F_"mix"$ 进行全局平均池化（GAP）得到向量 $bold(z)$，并通过一个线性分类器预测其在字典上的注意力分布（即当前样本属于哪种潜在任务模式的概率）：
...
在第三章中，本文提出了视觉感知路由模块，通过感知引导图像的纹理和边缘信息来实现任务特征的物理解耦。然而，正如引言所述，仅依靠视觉特征容易产生歧义。为了解决这一问题，本文将多模态提示生成模块（MPGM）生成的语义提示 $P_"sem"$ 引入到路由决策中，设计了融合多模态提示的动态路由模块（Multi-modal Guided Routing Module, MGRM）。
