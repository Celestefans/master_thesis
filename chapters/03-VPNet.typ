...
为了验证本章提出的面向一体化任务的视觉感知与动态路由GISR方法（VP-Net）的有效性，我们在全色锐化、磁共振图像超分及深度图超分三个典型任务上进行了广泛的实验。实验主要包含三个部分：首先通过对比实验，验证VP-Net在多任务混合训练设置下相较于基准模型及通用模型的性能优势；其次通过消融实验，深入探究视觉感知路由模块（VARM）的核心贡献及架构设计选择；最后通过可视化分析，直观展示动态路由机制在特征空间中的工作机理。
...
为了验证本章方法在多任务联合训练模式下的有效性，我们将 VP-Net 与近年来提出的主流一体化图像复原网络进行了全面对比，对比方法包括 Gridformer、Transweather、CAPTNet、AdaIR 以及 PromptIR。这些方法同样旨在通过单一网络处理多种复原任务，是检验 VP-Net 以及动态路由机制能否缓解任务干扰的理想基准。
...
#heading(level: 4, numbering: none)[One-by-One方法对比]

除了验证一体化训练的性能外，我们还在单任务独立训练（One-by-One）设置下评估了 VP-Net 的潜力，旨在探究其作为通用骨干网络是否具备与针对特定任务设计的专家模型相抗衡的能力。

在全色锐化任务中（@tab:VPNet_one-by-one_pansharpening），我们将 VP-Net 与当前领域内的专用 SOTA 方法进行了对比，包括 AWFLN、DISPNet、LAGConv、M3DNet、FusionMamba 以及基于扩散模型的 DifPan。结果显示，VP-Net 的性能优于大多数专用模型（如 FusionMamba），仅略逊于计算代价极高的扩散模型 DifPan。这表明即便不依赖特定任务的先验设计，仅凭强大的骨干网络与动态路由机制，VP-Net 也能达到领域前沿水平。

在磁共振图像超分任务中（@tab:VPNet_one-by-one_mri），我们引入了 MASA、SANet 和 DuDoNet 等专用模型作为对比。结果表明，VP-Net 在 2× 和 8× 倍率下的重建质量（PSNR 分别为 44.98 dB 和 35.47 dB）甚至超过了表现优异的 DuDoNet，在 4× 倍率下也与之相当，体现了模型在从医学影像中提取精细解剖结构方面的强大能力。

在深度图超分任务中（@tab:VPNet_one-by-one_depth），VP-Net 与 GeoDSR、DKN、SGNet 等方法进行了比较。在高倍率（16×）条件下，VP-Net 取得了最低的 RMSE（4.67），优于最新的 SGNet（4.77），证明了模型在处理极低分辨率输入时的鲁棒性。这些结果证实了 VPRM 模块不仅在多任务冲突场景下有效，在单任务场景下也能通过感知输入图像的视觉特性自适应地增强特征表达，具有良好的泛化性。

#include "../tables/03/single/Pansharpening.typ"

#include "../tables/03/single/MRI.typ"

#include "../tables/03/single/Depth.typ"
