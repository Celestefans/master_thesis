...
class MoE(nn.Module):
    ...
    def forward(self, x, guide):
        ...
        # 保存本次前向的 gating 矩阵，便于外部读取统计（shape: [B*H*W, num_experts]）
        try:
            # 保留原始 gates（不带梯度）以便后续分析
...
class VPNet(nn.Module):
    ...
    def forward(self, inp_tar, inp_guide): 
        ...
        inp_enc_level1, loss_tmp = self.spatial_routing_encoder_level1(inp_enc_level1, inp_guide_level1)
        ...
        inp_enc_level2, loss_tmp = self.spatial_routing_encoder_level2(inp_enc_level2, inp_guide_level2)
        ...
        inp_enc_level3, loss_tmp = self.spatial_routing_encoder_level3(inp_enc_level3, inp_guide_level3)
        loss_importance = loss_importance + loss_tmp
...
if __name__ == '__main__':
    ...
    model = WO_MMPGM(dim=22, num_blocks=[3, 4, 4, 5], prompt_num=32)
    model.cuda()
    model.eval()
